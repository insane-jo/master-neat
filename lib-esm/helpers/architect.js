    ...
    LSTM: function () {
        ...
        for (var i = 0; i < blocks.length; i++) {
            ...
            if (options.inputToDeep && i > 0) {
                var input_1 = inputLayer.connect(memoryCell, methods.connection.ALL_TO_ALL);
                inputGate.gate(input_1, methods.gating.INPUT);
            }
            // Optional connections
            if (options.memoryToMemory) {
                var input_2 = memoryCell.connect(memoryCell, methods.connection.ALL_TO_ELSE);
                inputGate.gate(input_2, methods.gating.INPUT);
            }
            if (options.outputToMemory) {
                var input_3 = outputLayer.connect(memoryCell, methods.connection.ALL_TO_ALL);
                inputGate.gate(input_3, methods.gating.INPUT);
            }
