...
var Network = /** @class */ (function () {
    ...
    Network.prototype.standalone = function () {
        ...
        for (i = this.input; i < this.nodes.length; i++) {
            ...
            activations.push(node_1.activation);
            states.push(node_1.state);
            var functionIndex = present.indexOf(node_1.squash.name);
            if (functionIndex === -1) {
                functionIndex = present.length;
                present.push(node_1.squash.name);
                functions.push(node_1.squash.toString());
            }
            var incoming = [];
            for (var j = 0; j < node_1.connections.in.length; j++) {
                var conn = node_1.connections.in[j];
            ...
            if (node_1.connections.self.weight) {
                var conn_2 = node_1.connections.self;
            ...
            var line1 = "S[".concat(i, "] = ").concat(incoming.join(' + '), " + ").concat(node_1.bias, ";");
            var line2 = "A[".concat(i, "] = F[").concat(functionIndex, "](S[").concat(i, "])").concat(!node_1.mask ? ' * ' + node_1.mask : '', ";");
            lines.push(line1);
            lines.push(line2);
        }
        var output = [];
