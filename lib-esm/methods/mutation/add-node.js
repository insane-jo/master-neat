...
var addNode = {
    ...
    callback: function (network) {
        ...
        if (connection) {
            ...
            from = network.nodes.find(function (node) { return node.type === NodeTypeEnum.input; });
            to = network.nodes.find(function (node) { return node.type === NodeTypeEnum.output; });
        }
        // Insert the new node right before the old connection.to
        var toIndex = network.nodes.indexOf(to);
        var node = new NodeElement(NodeTypeEnum.hidden);
