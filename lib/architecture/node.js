...
var NodeElement = /** @class */ (function () {
    ...
    NodeElement.prototype.activate = function (input) {
        ...
        for (var i = 0; i < this.connections.in.length; i++) {
            var connection_2 = this.connections.in[i];
            // Elegibility trace
            connection_2.elegibility = this.connections.self.gain * this.connections.self.weight *
                connection_2.elegibility + connection_2.from.activation * connection_2.gain;
            // Extended trace
            for (var j = 0; j < nodes.length; j++) {
                var node = nodes[j];
                var influence = influences[j];
                var index = connection_2.xtrace.nodes.indexOf(node);
                if (index > -1) {
                    connection_2.xtrace.values[index] = node.connections.self.gain * node.connections.self.weight *
                        connection_2.xtrace.values[index] + this.derivative * connection_2.elegibility * influence;
                }
                else {
                    // Does not exist there yet, might be through mutation
                    connection_2.xtrace.nodes.push(node);
                    connection_2.xtrace.values.push(this.derivative * connection_2.elegibility * influence);
                }
            }
        }
        return this.activation;
    ...
    NodeElement.prototype.connect = function (target, weight) {
        ...
        if (typeof target.bias !== 'undefined') { // must be a node!
            ...
            if (target === this) {
                ...
                var connection = new connection_1.default(this, target, weight);
            ...
            for (var i = 0; i < target.nodes.length; i++) {
                var connection = new connection_1.default(this, target.nodes[i], weight);
