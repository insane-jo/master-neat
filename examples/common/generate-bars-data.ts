...
function generateCandleData(numberOfPoints: number = 250): CandleData[] {
...
export const generateBarsData = async (): Promise<BarsData> => {
  const bars = generateCandleData(500);
  const volumesNotPrepared = generateLineData(500);

  const volumes: LineData[] = volumesNotPrepared.map((v, idx) => {
    return {
      value: v.value,
      time: bars[idx].time
    }
